...
module.exports = [
    ...
    },
    {
        ...
        handler: async (request, h) => {
            const { movieService } = request.services();
            try {
                await movieService.exportMoviesToCsv(request.auth.credentials);
                return h.response({ message: 'Export requested. You will receive the file by email.' }).code(202);
            } catch (error) {
                return h.response({ error: 'Failed to export movies' }).code(500);
            }
        }
    },
    {
